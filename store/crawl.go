...
func (s *Store) SetRobots(crawlID int64, robots string) error {
	res, err := s.db.Exec(`UPDATE crawls SET robots_txt = ? WHERE id = ?`, robots, crawlID)
	if err != nil {
...
func (s *Store) DeleteCrawl(id int64) error {
	res, err := s.db.Exec(`DELETE FROM crawls WHERE id = ?`, id)
	if err != nil {
...
func (s *Store) crawlExists(id int64) (bool, error) {
	var n int
	err := s.db.QueryRow(`SELECT 1 FROM crawls WHERE id = ?`, id).Scan(&n)
	if errors.Is(err, sql.ErrNoRows) {
		return false, nil
	}
	if err != nil {
